...
import '../../services/distribution_sync_service.dart';
import '../../services/local_db_service.dart';
import 'beneficiaire_form_screen.dart';
import 'ticket_scan_screen.dart';

class DistributionHomeScreen extends StatefulWidget {
  const DistributionHomeScreen({super.key});

  @override
  State<DistributionHomeScreen> createState() => _DistributionHomeScreenState();
}

class _DistributionHomeScreenState extends State<DistributionHomeScreen> {
  ...
  Future<void> _loadPlannings() async {
    ...
    final token = context.read<DistributionAuthService>().token;
    if (token == null) return;
    try {
      final res = await DistributionApiService.sync(token);
      if (res['success'] == true) {
        setState(() => _plannings = res['data'] ?? []);
      }
    } catch (_) {
      // Hors-ligne : on garde la liste précédente (vide si première ouverture)
    }
  ...
  Future<void> _sync({bool silent = false}) async {
    ...
    final result = await DistributionSyncService.syncPendingBeneficiaires(token);
    setState(() {
      _syncing = false;
    ...
    if (result.synced > 0 || !silent) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            result.synced > 0
                ? '${result.synced} bénéficiaire(s) synchronisé(s)${result.remaining > 0 ? ', ${result.remaining} restant(s)' : ''}'
                : (result.remaining > 0 ? '${result.remaining} en attente' : 'Aucun élément à synchroniser'),
          ),
  ...
  Widget build(BuildContext context) {
    final auth = context.watch<DistributionAuthService>();
        ...
        child: _loading
            ...
            : ListView(
                ...
                children: [
                  ...
                  const Text('Plannings de distribution', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Color(0xFFD84315))),
                  const SizedBox(height: 12),
                  if (_plannings.isEmpty)
                    const Padding(
                      padding: EdgeInsets.symmetric(vertical: 40),
                      child: Center(child: Text('Aucun planning assigné', style: TextStyle(color: Colors.grey))),
                    )
                  else
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          if (warehouse != null) ...[
            const SizedBox(height: 4),
            Text('Entrepôt : ${warehouse['name'] ?? ''}', style: const TextStyle(fontSize: 12, color: Colors.grey)),
          ],
          ...
          Row(
            children: [
              ...
              Text('$beneficiaireCount bénéficiaire(s)', style: TextStyle(fontSize: 12, color: Colors.grey.shade700)),
            ],
          ),
          const SizedBox(height: 12),
          SizedBox(
            ...
            child: OutlinedButton.icon(
              ...
              label: const Text('Nouveau bénéficiaire'),

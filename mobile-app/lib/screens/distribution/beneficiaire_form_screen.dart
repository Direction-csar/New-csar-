...
class _BeneficiaireFormScreenState extends State<BeneficiaireFormScreen> {
  ...
  static const _categories = ['Vulnérable', 'Religieux', 'Instruction', 'OAL', 'Spontané'];
  ...
  Future<void> _submit() async {
    ...
    if (token != null && connectivity != ConnectivityResult.none) {
      try {
        final res = await DistributionApiService.storeBeneficiaire(token, data);
        setState(() => _saving = false);
        if (!mounted) return;
        if (res['success'] == true) {
          _showSuccessAndClose('Bénéficiaire enregistré et synchronisé.');
        } else {
        ...
        setState(() => _saving = false);
        if (!mounted) return;
        _showSuccessAndClose('Enregistré hors-ligne. Sera synchronisé automatiquement.');
      }
    } else {
      await LocalDbService.savePendingBeneficiaire(data);
      setState(() => _saving = false);
          ...
          child: Column(
            ...
            children: [
              ...
              TextFormField(
                ...
                decoration: _inputDecoration('Quantité (kg) *', Icons.scale_outlined),
                validator: (v) {

...
class _TicketScanScreenState extends State<TicketScanScreen> {
  ...
  Future<void> _validate(String code) async {
    ...
    try {
      final res = await DistributionApiService.scan(token, code);
      if (!mounted) return;
      if (res['success'] == true) {
        _showResultDialog(
          success: true,
      ...
      if (!mounted) return;
      _showResultDialog(success: false, title: 'Erreur réseau. Vérifiez votre connexion.');
    }
    setState(() => _processing = false);
  }
        ...
        content: success && beneficiaire != null
            ? Column(
                ...
                children: [
                  ...
                  if (quantite != null) Text('Quantité : $quantite kg'),
                ],
              )
            : null,
        actions: [

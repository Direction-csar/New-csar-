...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        /* ── En-tête République ── */
        .rep-header { text-align: center; margin-bottom: 6px; }
        .rep-header .rep {
            font-size: 11pt; font-weight: bold;
            text-transform: uppercase; letter-spacing: 0.5px;
        }
        .rep-header .devise {
            font-size: 8.2pt; font-style: italic; color: #444; margin-top: 1px;
        }
        .rep-header .ministere {
            font-size: 8.6pt; font-weight: bold; text-transform: uppercase;
            margin-top: 5px; color: #1d6b3a;
        }
        .rep-header .org { font-size: 8.2pt; margin-top: 1px; }
        .sep { border: none; border-top: 1.5px solid #1d6b3a; margin: 8px 0 10px 0; }

        /* ── Titre ── */
        .doc-title { text-align: center; margin: 6px 0 12px 0; }
        .doc-title h1 {
            font-size: 13pt; text-transform: uppercase; margin: 0; letter-spacing: 0.5px;
        }
        .doc-title .ref { font-size: 8.5pt; color: #666; margin-top: 3px; }
        ...
        .info-table {
            ...
            margin: 6px 0 10px 0; font-size: 9.8pt;
        }
        .info-table td { border: 1px solid #cfcfcf; padding: 4px 8px; vertical-align: top; }
        .info-table td.label {
        ...
        .parties-title {
            ...
            color: #1d6b3a; margin: 8px 0 3px 0;
        }

        /* ── Articles ── */
        .article { margin: 7px 0; text-align: justify; }
        .article .art-title {
            font-weight: bold; text-transform: uppercase; font-size: 10pt; color: #14532d;
        }

        /* ── Signatures ── */
        .signatures { width: 100%; margin-top: 24px; border-collapse: collapse; }
        .signatures td { width: 50%; text-align: center; vertical-align: top; padding: 0 14px; }
        .signatures .sig-role { font-weight: bold; text-transform: uppercase; font-size: 9.6pt; }
        .signatures .sig-note { font-size: 8pt; color: #666; margin-top: 2px; }
        .signatures .sig-line {
            margin-top: 50px; border-top: 1px solid #333; padding-top: 3px; font-size: 8.5pt;
        }
...
<body>
    ...
    <div class="doc-title">
        ...
        <div class="ref">Réf. : {{ $reference ?? 'CTD-' . ($personnel->matricule ?? '____') . '-' . now()->format('Y') }}</div>
    </div>
    ...
    <div class="article">
        <span class="art-title">Article 1 — Engagement et objet.</span>
        L'Employeur engage l'Employé(e), qui accepte, dans le cadre d'un contrat de travail à durée
        <strong>déterminée</strong> régi par le Code du Travail du Sénégal et la Convention Collective applicable.
    </div>
    ...
    <div class="article">
        ...
        (lieu : {{ $personnel->localisation_region ?? 'Dakar' }}), et exécutera toutes tâches liées à cette fonction.
    </div>

    <div class="article">
        <span class="art-title">Article 3 — Durée du contrat.</span>
        Le présent contrat est conclu pour une durée déterminée allant du
        <strong>{{ optional($date_debut ?? $personnel->date_prise_service_csar)->format('d/m/Y') ?? '____' }}</strong>
        au <strong>{{ optional($date_fin)->format('d/m/Y') ?? '____' }}</strong>
        (soit {{ $duree ?? '____' }}). Il prend fin de plein droit à son terme, sans préavis ni indemnité de rupture.
    </div>

    <div class="article">
        <span class="art-title">Article 4 — Période d'essai.</span>
        Le contrat comporte une période d'essai de <strong>{{ $periode_essai ?? 'un (01) mois' }}</strong>,
        durant laquelle chaque partie peut y mettre fin sans préavis ni indemnité.
    </div>
    ...
    <div class="article">
        ...
        payable à terme échu, outre les indemnités prévues par les textes en vigueur.
    </div>

    <div class="article">
        <span class="art-title">Article 6 — Durée du travail.</span>
        La durée hebdomadaire de travail est fixée à <strong>quarante (40) heures</strong>, conformément à la
        législation en vigueur et à l'horaire de l'établissement.
    </div>

    <div class="article">
        <span class="art-title">Article 7 — Congés.</span>
        L'Employé(e) bénéficie, au prorata de la durée du contrat, d'un congé payé à raison de deux (02) jours
    ...
    <div class="article">
        <span class="art-title">Article 9 — Rupture anticipée.</span>
        Toute rupture avant le terme convenu, en dehors de la faute lourde ou de la force majeure, ouvre droit
        à des dommages et intérêts conformément aux dispositions du Code du Travail.
    </div>

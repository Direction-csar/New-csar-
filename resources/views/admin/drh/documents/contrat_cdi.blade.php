...
<body>
    ...
    <div class="travailleur">
        ...
        <p>Prénom : {{ $prenoms ?: '' }}</p>
        <p>Nom : {{ $nomFamille }}</p>
        <p>Date et Lieu de Naissance : {{ isset($personnel->date_naissance) ? $fmt($personnel->date_naissance) : '' }} à {{ $personnel->lieu_naissance ?? '' }}</p>
    ...
    <p style="text-align: justify;">
        Le lieu d'exécution du présent contrat de travail est situé à : Dakar, 22 Rue
        Amadou Assane NDOYE x Bérenger Ferraud
    </p>

    <div class="article-title">ARTICLE 5 :</div>
    <p style="text-align: justify;">
        {{ $civilite }} <strong>{{ $nomComplet }}</strong> est appelé à servir dans tous les services du CSAR
        partout où besoin sera.
    </p>

    <div class="article-title">ARTICLE 6 :</div>
    <p style="text-align: justify;">
        {{ $civilite }} <strong>{{ $nomComplet }}</strong> déclare formellement n'être lié
        actuellement à aucun employeur et libre de tout engagement.
    </p>
    ...
    <p style="text-align: justify;">
        ...
        d'un avenant qui sera notifié à l'intéressé.
    </p>
